# Where CogniDoc fits in the AI-coding-tool ecosystem

CogniDoc is sometimes confused with the per-tool convention files that AI coding tools auto-read at session start. This document clarifies the precise relationship.

---

## CogniDoc is content for slots, not a competitor of slots
...
CogniDoc is **content you can install into any of these slots**. A typical adoption pattern:

```bash
# 1. Install CogniDoc in your project
cd ~/my-project
/path/to/cognidoc/tools/init.sh . "My Project"

# 2. If you use Claude Code:
ln -s COGNIDOC.md CLAUDE.md

# 3. If you use Cursor:
ln -s COGNIDOC.md .cursorrules

# 4. If you use Windsurf:
ln -s COGNIDOC.md .windsurfrules

# 5. If you participate in the AGENTS.md emerging convention:
ln -s COGNIDOC.md AGENTS.md
```

Each tool reads its own slot, all slots resolve to the same `COGNIDOC.md` content, and the LLM operates under the same protocol regardless of which tool the human is using.

---

## What CogniDoc actually competes with

The real comparison is not CogniDoc vs `CLAUDE.md`. It is CogniDoc vs the **ad-hoc convention** that most engineers put inside `CLAUDE.md` (or `.cursorrules`, etc.) today: free-form prose with role descriptions, "always do X / never do Y" rules, project-specific notes, and occasional examples. The ad-hoc pattern works for small projects but has known limitations as projects grow.

| Dimension | Ad-hoc `CLAUDE.md` (or any slot) | CogniDoc-installed `CLAUDE.md` |
|-----------|-----------------------------------|--------------------------------|
...
| Cross-tool portability | Tool-specific (each tool's slot has bespoke content) | Same `COGNIDOC.md` content can be symlinked into any tool's slot |
...
**The honest summary:** the ad-hoc pattern is simpler. CogniDoc trades simplicity for structure, persistence, and auditability. Whether the trade is worth it depends on the project. A one-week prototype probably should not bother. A multi-month system with many LLM sessions per day and multiple human collaborators probably benefits.

---

## When NOT to use CogniDoc

Be honest with yourself. Skip CogniDoc when:
...
- You are using a tool where the slot has a strict size limit and a long bootloader does not fit. Some hosted tools cap rules files at a few KB; the CogniDoc bootloader is larger than that.

If any of those apply, ad-hoc `CLAUDE.md` (or `.cursorrules`, etc.) plus discipline is enough. Use CogniDoc when the cost of context loss between sessions is high enough to justify the structure.

---

## Practical compatibility

`COGNIDOC.md` is plain markdown. Any tool that reads markdown files at session start can be pointed at it. The only tool-specific consideration is what filename the tool watches, which is what the symlink pattern above resolves.

If your tool is not in the table above and you want to use CogniDoc with it: name the bootloader whatever your tool expects (or symlink, or copy), and feed it as the LLM's system prompt or first message. The protocol is the protocol; the filename is a delivery detail.
