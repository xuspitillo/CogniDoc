...
MandelDoc adapts to any project where LLMs participate in long-lived work. These seven scenarios illustrate how the system adds value across different domains.
...
**With MandelDoc:** The LLM reads MANDELDOC.md at session start and immediately knows: the project structure, active missions, recent decisions, and what comes next. Continuity without manual re-briefing.
...
**With MandelDoc:** All three share the same MEMORY.md and mission files via git. Each LLM session reads the same bootloader. Knowledge accumulates in one place instead of scattering across chat windows.
...
**With MandelDoc:** Each research thread becomes a mission. Papers and findings are tracked in progress logs. SBS beacons connect related concepts across threads. The signal registry tracks which threads are active, paused, or waiting on external input.
...
**With MandelDoc:** Each client project has its own MandelDoc instance. The consultant opens the relevant MANDELDOC.md, and the LLM has full context in seconds. No mental overhead for context switching.
...
**With MandelDoc:** MEMORY.md captures the project's identity, architecture, decisions, and lessons learned. Mission files document active work streams with full context. New contributors (and their LLMs) get oriented in minutes, not hours.
...
**With MandelDoc:** The PSS provides auditable state transitions with a formal 5-state machine. Handshakes create immutable narrative checkpoints. Mission logs record what was done, when, and why. The CPA pipeline ensures every operation follows a documented process from consultation through execution.
