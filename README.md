...
MandelDoc is a set of protocols, templates, and lightweight shell tools you install at the root of a project. Any LLM that reads markdown — Claude, GPT, Gemini, local models — can read MandelDoc's bootloader at session start, follow the conventions to navigate the project's documentation, update memory as work progresses, and hand off coherent state to the next session.

It is not prompt engineering, not a RAG framework, and not an agent library. It is a documentation and protocol layer those tools operate on top of. The closest comparison is to per-tool convention files (`CLAUDE.md`, `.cursorrules`, `.windsurfrules`, `AGENTS.md`): MandelDoc is **content** you can install in any of those slots — see [`docs/competitive-context.md`](docs/competitive-context.md) for the precise framing.

---

## The problem MandelDoc solves
...
MandelDoc addresses all five with a small set of protocols, templates, and conventions that any LLM can read on session start.

---

## What you get in this repository (Tier 1: Foundation)

This is the **public, open-source foundation** of MandelDoc. It contains:

- **Bootloader template** (`bootloader/MANDELDOC.md.template`) — the entry point any LLM reads first to understand the project.
...
- **Adoption documentation** (`docs/`) — getting started, concepts, comparisons (including [where MandelDoc fits vs `CLAUDE.md` / `.cursorrules` / `AGENTS.md`](docs/competitive-context.md)), glossary.
...
MandelDoc is **bring-your-own-LLM**:

| LLM type | How it works | File management |
|----------|-------------|-----------------|
| Code-integrated (Claude Code, Cursor, Windsurf) | Auto-reads `MANDELDOC.md` | Automatic |
...
```bash
# 1. Clone the repo
git clone https://github.com/xuspitillo/MandelDoc.git
cd MandelDoc

# 2. Initialize a new project from the templates
./tools/init.sh ~/my-project "My Project Name"

# 3. Feed MANDELDOC.md to your LLM (see docs/getting-started.md for your LLM type)
```
...
1. **The bootloader (`MANDELDOC.md`)** is the protocol the LLM reads first. It defines how to navigate, how to update memory, how to classify work, and how to verify coherence before committing changes.
...
| **MandelDoc** | Persistent, navigable project state that the above tools can operate on top of | (Use any of those tools INSIDE a MandelDoc-managed project; MandelDoc is the documentation layer, not a replacement for execution engines or retrieval systems) |

MandelDoc does not replace your existing stack. It **organizes** it.
...
The MandelDoc protocol specifications, templates, and tooling in this repository are licensed under Apache 2.0. You may use, modify, and redistribute them under those terms.

---

## Status

**Version:** 0.4.0 — Public Alpha
**Previous name:** This project was published under the name **CogniDoc** from v0.1.0 to v0.3.1. It was renamed to **MandelDoc** in v0.4.0 (2026-05-11) due to name collisions in the AI-doc tooling space. The protocols, templates, and design are otherwise unchanged.
